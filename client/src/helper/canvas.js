...
export function getFocuseDemention(element, padding) {
  ...
  // Handle pencil drawings differently
...
export function drawFocuse(element, context, padding, scale) {
  ...
  if (element.type === "pencil") {
    ...
    corners.forEach((corner) => {
      context.roundRect(corner.x, corner.y, square, square, 3 / scale);
    });
    context.fill();
    context.stroke();
    context.closePath();
...
export function draw(element, context) {
  // Handle pencil drawings
  if (element.type === "pencil") {
    shapes.pencil(element.points, context, {
  ...
  // Original drawing logic for other shapes
  context.beginPath();
  const {
    tool,
    x1,
...
export function inSelectedCorner(element, x, y, padding, scale) {
  // Handle pencil drawings differently
  if (element.type === "pencil") {
    padding = 0; // Use simpler hit detection for pencil drawings
...
export function drawPencil(element, context) {
  shapes.pencil(element.points, context, {
